...
- Note tat you are able to use the search function drill down to the specifics. I will type in *ssh* and see what comes up.
...
- Here above, it even shows the security events for the Linode VM that the Wazuh server is on. I can see 1601 authentication failures. I purposely left the firewall off for the Linode VM to demonstrate the insights one can gain from this situation. If i scroll down to the list of security alerts and click on the most recent, the first one is an *attempt to login using a non-existent user*
...
- The security event shows that someone in the U.K. is attempting to brute force into the machine via SSH. The alert has the MITRE attack Tactics and techniques used by the attacker. It also reveals geolocation, timestamps, incompliance with pcidss. (I will go turn the firewall on now)



- Wazuh also has File integrity monitoring. This is very powerful for Windows(The default scan time is 24 hours). Since i just set this machine up, I will have to configure the timing of the scans to monitor the file integrity for my Windows VM. To do so, I wll head to the config file. The same one I had to for the IP address. I will hit *CTL+F* and type in *syscheck* to find the file integrity monitoring.
...
-  Wazuh does have an inventory of registry keys that it automatically monitors, so I can add specific ones that i want to have seen. I will add a custom one to it. To do that, I will open up reg edit.
...
- Monitoring the windows registry is useful because the Windows registry has the configuration settings for your machine. If things are changed here, things are changing everywhere else. Something to look out for in the windows registry is maleware. What maleware will do is add keys to your run folfer. *HKEY_LOCAL_MACHINE\SOFTWARE\Classes\Microsoft\Windows\CurrentVersion\run*. These keys are changed when you add a new startup application for your system. This is how you monitor that. I will go and add a new key to that folder.
...
- Something else cool in Wazuh is Active responses. This is also super powerful. The "Acctive response" feature can setup automated responses that trigger when certain alerts are generated. This helps in quickly mitigating threats without manual intervention. Here is how I will do that.
...
- Now, I will configure Wazuh actively respond to failed SSH login attempts and add it to the block list. I will do so by heading back to the dashboard in Wazuh server and click *Wazuh* > *Management* > *Configuration* > *Edit configuration*.
...
- Another feature to appreciate is "Vulnerabilities". This is not enabled by default in the the Manager, however, they are the agents. To enable this on the manager, I'll go to *Wazuh* > *Management* > *Configuration* > *Edit configuration*. Hit *CTL+F* and type *Vulnerabilities* and I am going to search for Vulnerability detector.

   ![image](https://github.com/user-attachments/assets/c64add98-17a8-4465-a047-83542bc5b9b9)


- Right there where it says *<enabled>no</enabled>* I will change it to yes. Click on save and restart manager.
